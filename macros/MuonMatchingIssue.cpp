...
void testMuonMatchSelection(const Dataset &dataset){
    ...
    //Seleçao do trigger matching 'L2SingleMu12'
    ULong64_t triggerMatch = 1ULL << 7;

    // Antes do event loop
    int goodCandidatesCount = 0;
...
for (Long64_t i = 0; i < nEvents; ++i) {
    ...
    for (Short_t j = 0; j < Reco_Dimuon_size; ++j) {
        ...
        if(!(Reco_Dimuon_trig[j] & triggerMatch)) continue;
    
        goodCandidatesCount+=1;
    }
}

    std::cout << "Numero de good Candidates = " << goodCandidatesCount << std::endl;
