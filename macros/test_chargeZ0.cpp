...
void pT_asymmetry(const Dataset &dataset){
    ...
    for(Long64_t i = 0; i < nEvents; ++i){ //Loop through all EVENTS in the CHAIN.
        ...
        for(Short_t j = 0; j < Reco_Dimuon_size; ++j){ //Loop through all reco dimuon candidates of event i.
            ...
            if (Reco_Dimuon_sign[j] != 0){
                std::cout << "Found a dimuon with charge different than zero in event \n"
                << i
                << "Charge = " << Reco_Dimuon_sign[j]
                << std::endl;
                chargedZCounter += 1;
            }
    ...
    std::cout << "Total n of charged dimuon candidates -> " << chargedZCounter << std::endl;
