/*
Calculates fraction of dimuons having muons above 100 GeV.
*/
...
void makeGoodSelection(const Dataset& dataset, TFile* outputFile){
    ...
    int DimuonHasMuonAbove100GeV = 0; //Counter for muons above 100 GeV.
    
    int nMuPlusAbove100GeV = 0; //Counter for muons above 100 GeV (mu+).
    int nMuMinusAbove100GeV = 0; //Counter for muons above 100 GeV (mu-).
    int bothMuonsAbove100GeV = 0; //Counter for dimuons with both muons above 100 GeV.
    ...
    for(Long64_t i = 0; i < nEvents; ++i){//Loop through all EVENTS in the CHAIN.
        ...
        for(Short_t j = 0; j < Reco_Dimuon_size; ++j){ //Loop through all reco dimuon candidates of event i.
            ...
            if(ptplus > 100. || ptminus > 100.) {
            
                if(ptplus > 100. && ptminus <= 100.) nMuPlusAbove100GeV++; //Increment counter for muons above 100 GeV.
                if(ptminus > 100. && ptplus <= 100.) nMuMinusAbove100GeV++; //Increment counter for muons above 100 GeV.
                if(ptplus > 100. && ptminus > 100.) bothMuonsAbove100GeV++; //Increment counter for dimuons with both muons above 100 GeV.

                DimuonHasMuonAbove100GeV++; //Increment counter for muons above 100 GeV.
            }
    ...
    std::cout << "> Number of dimuons with at least one muon above 100 GeV = " << DimuonHasMuonAbove100GeV << std::endl;
    std::cout << "> Number of dimuons mu+ > 100 GeV (only mu+) = " << nMuPlusAbove100GeV << std::endl;
    std::cout << "> Number of dimuons mu- > 100 GeV (only mu-) = " << nMuMinusAbove100GeV << std::endl;
    std::cout << "> Number of dimuons with both muons above 100 GeV = " << bothMuonsAbove100GeV << std::endl;
    std::cout << "Check sum: " << nMuPlusAbove100GeV + nMuMinusAbove100GeV + bothMuonsAbove100GeV << " = " << DimuonHasMuonAbove100GeV << std::endl; 
