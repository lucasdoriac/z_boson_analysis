...
void MakeNormalizedDistPtRelDiff_PbPb_vs_ppRef_CentBin(TFile* inputFile, double lowCent, double highCent){
    ...
    TH1D* h_PbPb = h_PbPb_original->ProjectionY(
        ("h_PbPb_" + histName + "_Cent_" + std::to_string(static_cast<int>(lowCent)) + "_" + std::to_string(static_cast<int>(highCent))).c_str(),
        binLow, binHigh);
    ...
    h_PbPb->GetXaxis()->SetTitle("#Delta p_{T}");
    ...
    std::string PbPb_mean_text = "#LT #Delta p_{T} #GT_{PbPb} = " + std::to_string(PbPb_mean) + " #pm " + std::to_string(PbPb_mean_error);
    std::string ppRef_mean_text = "#LT #Delta p_{T} #GT_{ppRef} = " + std::to_string(ppRef_mean) + " #pm " + std::to_string(ppRef_mean_error);
