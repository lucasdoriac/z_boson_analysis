...
void make_hist_pt_plmi(){
	...
	TProfile *prof_pt_plmi = new TProfile("prof_pt_plmi","Profile of p_{T}(#mu^{+}) vs p_{T}(#mu^{-})", 100, 0, 100, 100, 0, 100);
	prof_pt_plmi->SetStats(0);
	...
	for(Long64_t i = 0; i < nEvents; ++i){// Loop over events...
		...
		if(Reco_QQ_size > 0){//If event has dimuon candidate...
			...
			for(Short_t j = 0; j < Reco_QQ_size; ++j){//Maybe event has more than one candidate...
				...
				if(Reco_QQ_4mom_m->at(j) < 95. && Reco_QQ_4mom_m->at(j) > 85.){//If j-th candidate has mass in [85,95] GeV...
					...
					prof_pt_plmi->Fill(ptPlus, ptMinus);
				}
			}
		}
	}
	...
	hist_pt_plmi->GetXaxis()->SetRangeUser(10., 100.);
	hist_pt_plmi->GetYaxis()->SetRangeUser(10., 100.);
	hist_pt_plmi->SetStats(0);

	hist_pt_plmi->Draw("COLZ");
  	prof_pt_plmi->Draw("E SAME");
	c->Update();
	c->SaveAs("output2.png");
